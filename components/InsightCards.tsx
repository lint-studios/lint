...
function InsightCard({ title, description, icon, iconColor, items }: InsightCardProps) {
  return (
    <section className="panel">
      ...
      <p className="subtle mb-5">{description}</p>
      
      <div className="space-y-3">
        {items.map((item, index) => (
          <div key={index} className="panel-card flex items-center justify-between">
            <div>
              <div className="font-medium">{item.title}</div>
              <div className="subtle text-sm">{item.description}</div>
            </div>
            {item.metric && (
              <span className={item.status === "positive" ? "badge-pos" : "bg-gray-100 text-gray-700 px-2 py-1 rounded-full text-sm"}>
                {item.metric}
              </span>
            )}
          </div>
